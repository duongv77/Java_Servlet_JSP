				...
				<table class="table">
					...
					<tbody>
						<c:forEach items="${ listVideo }" var="listVideo">
							<tr>
								<th scope="row">${ listVideo.id }</th>
								<td>${ listVideo.title }</td>
								<td>${ listVideo.poster }</td>
								<td>${ listVideo.views }</td>
								<td>${ listVideo.active==1? "On" : "Off" }</td>

								<td>
									<div class="row">
										<a href="/ASSM/admin/baiviet/update?id=${ listVideo.id }">
											<button type="button" class="btn btn-success">
												<i class="fa fa-pencil-square-o" aria-hidden="true"></i>
											</button>
										</a>
										 <a class="mx-auto"
											href="/ASSM/admin/baiviet/delete?id=${ listVideo.id }">
